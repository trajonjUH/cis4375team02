...
export default {
  ...
  created() {
    ...
    axios.get(`${apiURL}/order/id/${this.$route.params.id}`).then((res) => {
      this.event = res.data
      this.event.date = this.formattedDate(this.order.date)
  ...
  methods: {
    ...
    handleEventUpdate() {
      ...
      this.alertMessage = "Order updated";
      axios.put(`${apiURL}/order/update/${this.id}`, this.event).then(() => {
    ...
    editClient(clientID) {
      ...
      this.alertMessage = "Order updated";
      this.$router.push({ name: 'updateorder', params: { id: orderID } })
    },
    eventDelete() {
      this.alertMessage = "Order deleted";
